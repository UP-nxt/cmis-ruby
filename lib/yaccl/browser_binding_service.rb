...
module YACCL
  class CMISRequestError < RuntimeError
    attr_reader :code
    attr_reader :server_message
    attr_reader :required_params
    attr_reader :optional_params

    def initialize(code, server_message, required_params, optional_params)
      @code = code
    ...
    class BrowserBindingService
      ...
      def perform_request(required_params={}, optional_params={})
        ...
        unless (200...300).include?(response.code.to_i)
          #params_message = "\nRequest:\nRequired params: #{required_params.to_json}\nOptional params: #{optional_params.to_json}"
          if result.is_a?(Hash) && result.has_key?(:exception)
            raise CMISRequestError.new(response.code, result[:message], required_params, optional_params)#, "#{response.code} -- #{result[:exception]} -- #{result[:message]}#{params_message}"
          else
            raise CMISRequestError.new(response.code, result, required_params, optional_params)#, "#{response.code} -- #{result}#{params_message}"
          end
        end
